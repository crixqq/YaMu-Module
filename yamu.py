...
# ---- PERFORMANCE HELPERS (async replacements for blocking requests) ----
YM_CLIENT = None

async def get_client(token: str):
    """Return a single initialized yandex_music.Client (cached)."""
    global YM_CLIENT
    if YM_CLIENT is None:
        loop = asyncio.get_event_loop()
        YM_CLIENT = await loop.run_in_executor(None, lambda: yandex_music.Client(token).init())
    return YM_CLIENT


async def fetch_bytes(url: str) -> bytes:
    """Fetch bytes asynchronously via aiohttp."""
...
class YaMusicMod(loader.Module):
    ...
    async def autobio(self):
        # Autobio expects a valid token in config
        ...
        try:
            ...
            now = await self.__get_now_playing(token, client)
        except Exception as e:
            logger.exception("Error while fetching now playing for autobio: %s", e)
            return

        limit = 140 if getattr(self, '_premium', False) else 70
        out = (self.config['no_playing_bio'] or '')[:limit]

        if now and (not now.get('paused', True)):
            artist = ", ".join(now['track'].get('artist', [])) if now['track'].get('artist') else ''
            title = now['track'].get('title', '')
            template = self.config['autobio'] or "{artist} - {title}"
            try:
                out = template.format(artist=artist, title=title)[:limit]
            except Exception:
                out = f"{artist} - {title}"[:limit]

        # Update profile with floodwait handling
        try:
            await self._client(telethon.functions.account.UpdateProfileRequest(about=out))
        except telethon.errors.rpcerrorlist.FloodWaitError as e:
            logger.info(f"Sleeping {max(e.seconds, 60)} because of floodwait")
            await asyncio.sleep(max(e.seconds, 60))
        except Exception:
            logger.exception("Failed to update autobio")
    ...
    async def ynowcmd(self, message: telethon.types.Message):
        ...
        client = await get_client(self.config['token'])
        ...
        try:
            data = await fetch_bytes(now['track']['download_link'])
            audio = io.BytesIO(data)
            audio.name = "audio.mp3"
        except Exception:
            logger.exception("Failed to download audio")
            return await utils.answer(message, "<emoji document_id=5778527486270770928>❌</emoji> <b>Failed to download track</b>")
    ...
    async def ynowbcmd(self, message: telethon.types.Message):
        ...
        # create banner (async)
        try:
            file = await self.__create_banner(
                now['track']['title'], now['track']['artist'],
                now['duration_ms'], now['progress_ms'],
                now['track']['img']
            )
        except Exception:
            logger.exception("Failed to create banner")
            return await utils.answer(message, "<emoji document_id=5778527486270770928>❌</emoji> <b>Failed to generate banner</b>")
    ...
    async def ylyricscmd(self, message: telethon.types.Message):
        ...
        try:
            ...
            text = await fetch_text(lyrics.download_url)
            await utils.answer(
            ...
            await utils.answer(message, "<emoji document_id=5778527486270770928>❌</emoji> <b>Failed to get lyrics</b>")
    ...
    async def ysearchcmd(self, message: telethon.types.Message):
        ...
        try:
            data = await fetch_bytes(link)
            audio = io.BytesIO(data)
            audio.name = "audio.mp3"
        except Exception:
            logger.exception("Failed to download search result audio")
            return await utils.answer(message, "<emoji document_id=5778527486270770928>❌</emoji> <b>Failed to download track</b>")
    ...
    async def __create_banner(
        self,
        title: str, artists: list,
        duration: int, progress: int,
        track_img_url: str
    ):
        """Async banner creator: fetches fonts and cover asynchronously."""
        w, h = 1920, 768
        # fetch fonts and cover asynchronously
        try:
            font_bold_bytes, font_reg_bytes, font_time_bytes = await asyncio.gather(
                fetch_bytes("https://raw.githubusercontent.com/kamekuro/assets/master/fonts/Onest-Bold.ttf"),
                fetch_bytes("https://raw.githubusercontent.com/kamekuro/assets/master/fonts/Onest-Regular.ttf"),
                fetch_bytes("https://raw.githubusercontent.com/kamekuro/assets/master/fonts/Onest-Bold.ttf")
            )
            track_cover_bytes = await fetch_bytes(track_img_url)
        except Exception:
            logger.exception("Failed to fetch assets for banner, falling back to requests")
            # fallback to requests (sync) if aiohttp failed for some reason
            try:
                font_bold_bytes = requests.get("https://raw.githubusercontent.com/kamekuro/assets/master/fonts/Onest-Bold.ttf").content
                font_reg_bytes = requests.get("https://raw.githubusercontent.com/kamekuro/assets/master/fonts/Onest-Regular.ttf").content
                font_time_bytes = font_bold_bytes
                track_cover_bytes = requests.get(track_img_url).content
            except Exception:
                logger.exception("Complete failure fetching banner assets")
                raise

        title_font = ImageFont.truetype(io.BytesIO(font_bold_bytes), 80)
        art_font = ImageFont.truetype(io.BytesIO(font_reg_bytes), 55)
        time_font = ImageFont.truetype(io.BytesIO(font_time_bytes), 36)

        # Gen banner (bg)
        track_cov = Image.open(io.BytesIO(track_cover_bytes)).convert("RGBA")
        ...
        # Drawing status bar
        try:
            progress_value = int(1072*(progress/duration)) if duration else 0
        except Exception:
            progress_value = 0
        draw.rounded_rectangle([768, 650, 768+1072, 650+15], radius=15//2, fill="#A0A0A0")
        draw.rounded_rectangle([768, 650, 768+progress_value, 650+15], radius=15//2, fill="#FFFFFF")
